...
FEATURE_COLUMNS = [
    "ghi_cs", "clear_sky_index", "cloud_pct", "temp_c", "wind_ms",
    "hour_sin", "hour_cos", "doy_sin", "doy_cos", "ghi_lag1", "ghi_lag24",
]
...
def build(df: pd.DataFrame, site: Site) -> pd.DataFrame:
    ...
    No look-ahead: lag features use only past values. Rows with NaN lags are dropped.
    """
    ...
    out["ghi_cs"] = ghi_cs
    out["clear_sky_index"] = np.where(ghi_cs > 1.0, ghi / np.maximum(ghi_cs, 1e-6), 0.0)
